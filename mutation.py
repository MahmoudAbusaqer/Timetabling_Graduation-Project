...
def neighbour(chromosome):
    ...
    found = False
    pairs = []
    # iterate over the 60 objects (gene) and get the allowed class from the generated timetable
        ...
        for k in range(len(chromosome[2][classroom])):
            ...
            if chromosome[2][classroom][k] == 0 and k % 12 + length <= 12:
                ...
                if c == length:
                    ...
                    if k != 59:
                        # store all the times and classroom that can be available for this class time
    ...
    if not found:
        ...
        # find a time (period) from the start of the day till the end - the length of the class time
        if day == 4:  # Friday 8pm is reserved for free hour
...
# Find two candidates that can be swapped (constraints are type of classroom and length, because of overlapping days)
    while not satisfied:
        ...
        if first['Zadata_ucionica'] in second['Ucionica'] and second['Zadata_ucionica'] in first['Ucionica']\
            ...
            if first['Zadato_vreme'] + int(second['Trajanje']) != 60 \
                    and second['Zadato_vreme'] + int(first['Trajanje']) != 60\
                    and first != second:
                satisfied = True
        c += 1
